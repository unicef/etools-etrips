describe('Reports - Action Points', function() {
    ...
    it('should be able have a list of the newly created action point ', function(){
        ...
        element.all(by.css('ion-view[nav-view="active"] a.item-content')).count().should.eventually.equal(2);

        // validate new action point
        element.all(by.css('ion-view[nav-view="active"] a.item-content p')).get(1).getText().should.eventually.equal(description.substring(0, 254));
